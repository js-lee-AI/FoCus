...
DPR_PASSAGE_ENCODER = SentenceTransformer(
    "facebook-dpr-ctx_encoder-single-nq-base", device="cuda"
)
DPR_QUERY_ENCODER = SentenceTransformer(
    "facebook-dpr-question_encoder-single-nq-base", device="cuda"
)
STS_MODEL = SentenceTransformer(
    "sentence-transformers/multi-qa-MiniLM-L6-dot-v1", device="cuda"
)
...
class DPR:
    ...
    def similarities(self, question_text):
        knowledge_candidate_embeddings = self.passage_encoder.encode(
            self.knowledge_candidate, show_progress_bar=False
        )
        question_embedding = self.query_encoder.encode(
            question_text, show_progress_bar=False
        )
        scores = (
            util.dot_score(knowledge_candidate_embeddings, question_embedding)
            .squeeze()
            .tolist()
        )
    ...
    def example(self):
        ...
        passage_embeddings = self.passage_encoder.encode(
            passages, show_progress_bar=False
        )
...
class STS:
    ...
    def similarities(self, question_text):
        knowledge_candidate_embeddings = self.model.encode(
            self.knowledge_candidate, show_progress_bar=False
        )
...
if __name__ == "__main__":
    ...
    print(
        f"TFIDF: {sorted(sim_method.similarities(query), key=lambda x: x[1], reverse=True)}"
    )
    ...
    print(
        f"BM25: {sorted(sim_method.similarities(query), key=lambda x:x[1], reverse=True)}"
    )
    ...
    sim_method.add_document(passages)
    print(
        f"DPR: {sorted(sim_method.similarities(query), key=lambda x:x[1], reverse=True)}"
    )
    ...
    sim_method.add_document(passages)
    print(
        f"STS: {sorted(sim_method.similarities(query), key=lambda x:x[1], reverse=True)}"
    )
