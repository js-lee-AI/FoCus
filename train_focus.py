...
def train():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt2",
        help="pre-trained model path for decoder only models",
    )  # gpt2-medium
    parser.add_argument(
        "--bart_model_path",
        type=str,
        default="facebook/bart-base",
        help="pre-trained model path for encoder-decoder models",
    )  # facebook/bart-large
    parser.add_argument(
        "--train_dataset_path",
        type=str,
        default="data/train_focus.json",
        help="Path or url of the dataset.",
    )
    parser.add_argument(
        "--train_dataset_cache",
        type=str,
        default="data/focus_cache.tar.gz",
        help="Path or url of the dataset cache",
    )
    parser.add_argument(
        "--dev_dataset_path",
        type=str,
        default="data/valid_focus.json",
        help="Path or url of the dataset.",
    )
    parser.add_argument(
        "--dev_dataset_cache",
    ...
    parser.add_argument(
        "--ps_coef", type=float, default=1.0, help="Coefficient for persona loss"
    )
    parser.add_argument(
        "--kn_coef", type=float, default=1.0, help="Coefficient for knowledge loss"
    )
    parser.add_argument(
        "--lm_coef", type=float, default=10.0, help="Coefficient for LM loss"
    )
    parser.add_argument(
        "--max_history",
        type=int,
    ...
    parser.add_argument(
        "--inference",
        action="store_true",
        help="If true, inference with gold knowledge",
    )
    parser.add_argument(
        "--test_infer", action="store_true", help="If true, test inference"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--gpu_start_num", type=int, default=1, help="Start number of GPU"
    )
    parser.add_argument(
    ...
    wandb.init(
        config=args,
        name=f"{args.model_name}-{args.retrieval}",
        project="Training-FoCus",
    )
    ...
    def update(engine, batch):
        ...
        loss = (
            lm_loss * args.lm_coef
            + knowledge_loss * args.kn_coef
            + persona_loss * args.ps_coef
        ) / args.gradient_accumulation_steps

        wandb.log(
            data={
                f"train/lm_loss": lm_loss,
                f"train/ppl": math.exp(lm_loss),
                f"train/knowledge_loss": knowledge_loss,
                f"train/persona_loss": persona_loss,
                f"train/entire_loss": loss,
            }
        )
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            if model.config.model_type == "gpt2":
                ...
                lm_logits, knowledge_logits, persona_logits = (
                    output[0],
                    output[1],
                    output[2],
                )
    ...
    metrics = {
        ...
        "knowledge_loss": Loss(
            torch.nn.CrossEntropyLoss(), output_transform=lambda x: (x[0][1], x[1][1])
        ),
        "persona_loss": Loss(
            torch.nn.BCEWithLogitsLoss(), output_transform=lambda x: (x[0][2], x[1][2])
        ),
    ...
    metrics.update(
        {
            "average_lm_loss": MetricsLambda(
                average_distributed_scalar, metrics["lm_loss"], args
            ),
            "average_knowledge_loss": MetricsLambda(
                average_distributed_scalar, metrics["knowledge_loss"], args
            ),
            "average_persona_loss": MetricsLambda(
                average_distributed_scalar, metrics["persona_loss"], args
            ),
            "average_Knowledge_acc": MetricsLambda(
                average_distributed_scalar, metrics["Knowledge_acc"], args
            ),
            "average_Persona_acc": MetricsLambda(
                average_distributed_scalar, metrics["Persona_acc"], args
            ),
        }
    )
    ...
    if args.local_rank in [-1, 0]:
        ...
        dir_name = (
            str(os.path.basename(__file__))[:-3]
            + "_"
            + args.model_name
            + "_"
            + args.flag
        ...
        tb_logger.attach(
            evaluator,
            log_handler=OutputHandler(
                tag="validation", metric_names=list(metrics.keys())
            ),
            event_name=Events.ITERATION_STARTED,
        )
        tb_logger.attach(
