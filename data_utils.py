...
from python_tf_idf.tfidf import TfIdf, BM25, DPR, STS
...
ATTR_TO_SPECIAL_TOKEN = {
    "additional_special_tokens": ["<machine>", "<human>", "<persona>", "<knowledge>"]
}
BART_MODEL_INPUTS = [
    "input_ids",
    "decoder_input_ids",
...
def pad_dataset_gpt2_inctxt(dataset, padding=0):
    ...
    max_l_knowledge_cans = max(
        [len(i) for x in new_dataset["knowledge_candidates"] for i in x]
    )
    max_l_tot_knowledge = max([len(i) for x in new_dataset["tot_knowledge"] for i in x])
    max_l_persona_cans = max(
        [len(i) for x in new_dataset["persona_candidates"] for i in x]
    )
    ...
    for name in GPT2_PADDED_INPUTS:
        new_dataset[name] = [
            ...
            for x in new_dataset[name]
        ]
    new_dataset["reply"] = [
        x + [padding if name != "lm_labels" else -100] * (max_l_reply - len(x))
        for x in new_dataset["reply"]
    ]
    ...
    for i, knowledges in enumerate(new_dataset["knowledge_candidates"]):
        ...
        for candidates in knowledges:
            padded_candidate = candidates + [padding] * (
                max_l_knowledge_cans - len(candidates)
            )
    ...
    for i, personas in enumerate(new_dataset["persona_candidates"]):
        ...
        for candidates in personas:
            padded_candidate = candidates + [padding] * (
                max_l_persona_cans - len(candidates)
            )
            candidates_list.append(padded_candidate)
    ...
    for i, tot_kn in enumerate(new_dataset["tot_knowledge"]):
        ...
        for candidates in tot_kn:
            padded_candidate = candidates + [padding] * (
                max_l_tot_knowledge - len(candidates)
            )
            candidates_list.append(padded_candidate)
    ...
    new_dataset["dialog"] = [
        x + [padding] * (max_l_dialog - len(x)) for x in new_dataset["dialog"]
    ]
    new_dataset["dialog_tti"] = [
        x + [padding] * (max_l_dialog - len(x)) for x in new_dataset["dialog_tti"]
    ]
...
def pad_dataset_bart_inctxt(dataset, padding=1):
    ...
    new_dataset["input_ids"] = [
        x + [padding if name != "lm_labels" else -100] * (max_enc_l - len(x))
        for x in new_dataset["input_ids"]
    ]
    new_dataset["reply"] = [
        x + [padding if name != "lm_labels" else -100] * (max_l_reply - len(x))
        for x in new_dataset["reply"]
...
def choose_knowledge(args, knowledge, knowledge_text, question, question_text):
    # print('#'*200)
    # print(knowledge[:3])
    #
    # print(question_text)
    ...
    if args.retrieval == "tfidf" or args.retrieval is None:
        # print('@' * 200)
        table = TfIdf()
        ...
        # print(f"results_1: {results}")
        results = sorted(results, key=lambda x: x[1], reverse=True)
        result_idx = [i[0] for i in results[:5]]
        chosen_knowledge = [knowledge[ri] for ri in result_idx]
        # print(f"results_2: {results}")
        # print(f"results_idx: {result_idx}")
        # print(f"chosen_knowledge: {chosen_knowledge}")
        # print('#' * 200)

    elif args.retrieval == "bm25":
        table = BM25()
        table.add_document(knowledge_text)

        results = table.similarities(question_text)
        results = sorted(results, key=lambda x: x[1], reverse=True)
        result_idx = [i[0] for i in results[:5]]
        chosen_knowledge = [knowledge[ri] for ri in result_idx]
    ...
    elif args.retrieval == "sbert":
        pass

    return chosen_knowledge


def build_input_from_segments_bart_inctxt(
    args,
    persona,
    knowledge,
    knowledge_text,
    ...
    machine_st, human_st, persona_st, knowledge_st = tokenizer.convert_tokens_to_ids(
        SPECIAL_TOKENS
    )
    ...
    history = [
        [human_st if i % 2 == 0 else machine_st] + s for i, s in enumerate(history)
    ]
    reply = reply
    ...
    if inference == False:
        if len(knowledge) > 1:
            ...
            chosen_knowledge = choose_knowledge(
                args, knowledge, knowledge_text, question, question_text
            )
        else:
    ...
    instance["persona_candidates"] = [
        [bos] + can + [persona_st] + [eos] for can in persona_cans
    ]
    instance["persona_can_idx"] = [
        len(can) - 1 for can in instance["persona_candidates"]
    ]
    instance["persona_grounding"] = persona_grounding
    instance["knowledge_candidates"] = [
        [bos] + can[:100] + [knowledge_st] + [eos]
        if len(can) > 100
        else [bos] + can + [knowledge_st] + [eos]
        for can in knowledge_cans
    ]
    instance["knowledge_can_idx"] = [
        len(can) - 1 for can in instance["knowledge_candidates"]
    ]
    ...
    instance["tot_knowledge_token_ids"] = [
        [knowledge_st] * max_tot_k_token_ids
        + [tokenizer.pad_token_id] * (100 - max_tot_k_token_ids)
        for _ in range(5)
    ]
...
def build_input_from_segments_gpt2_inctxt(
    args,
    persona,
    knowledge,
    knowledge_text,
    ...
    if lm_labels:
        instance["lm_labels"] = (
            ([-100] * sum(len(s) for s in dialog[:-1])) + [-100] + dialog[-1][1:]
        )
    instance["persona_candidates"] = [
        can + [persona_st] + [eos] for can in persona_cans
    ]
    instance["persona_can_idx"] = [
        len(can) - 1 for can in instance["persona_candidates"]
    ]
...
def get_data_loaders(args, tokenizer, generation=False):
    ...
    plan = get_dataset_only_train_dev(
        tokenizer,
    ...
    for dataset_name, dataset in plan.items():
        ...
        if generation == True:
            ...
            testset = False
        for dialog in tqdm(
            dataset, total=len(dataset), desc=f"preprocess-{args.retrieval}"
        ):
            ID = dialog["dialogID"]
            persona = dialog["persona"]
            knowledge = dialog["knowledge"]
            knowledge_text = [tokenizer.decode(kn) for kn in knowledge]
            utterance = dialog["utterance"]
            # print('**'*200)
            # print(knowledge)
            # print(tokenizer.decode(knowledge[0]))
            # print(tokenizer.decode(knowledge[1]))
            for i, utt in enumerate(utterance):
                ...
                if model_name == "GPT2" or model_name == "transformer-decoder":
                    instance = build_input_from_segments_gpt2_inctxt(
                        args,
                        persona,
                        knowledge,
                        knowledge_text,
...
def get_testdata_loaders(args, tokenizer, generation=True):
    ...
    plan = get_dataset_only_test(
        tokenizer, args.test_dataset_path, args.test_dataset_cache
    )
